...
<div id="app" class="d-flex flex-column min-vh-100">
                ...
                <form class="d-flex mx-lg-4 flex-grow-1" role="search" action="{{ url('/search') }}" method="GET"
                    ...
                    <div class="input-group">
                        ...
                        <input
                            ...
                            value="{{ request('q') }}">
                        <button type="submit" class="btn btn-primary d-lg-none rounded-end-pill px-4 text-white">
                            Найти
                        </button>
                    </div>
                </form>
                            ...
                            <a id="navbarDropdown"
                               ...
                               aria-expanded="false" v-pre>
                                @if(Auth::user()->avatar)
                                    ...
                                    <div
                                        class="avatar bg-primary text-white rounded-circle d-none align-items-center justify-content-center fw-bold shadow-sm"
                                        style="width: 32px; height: 32px; font-size: 14px;">
                                        {{ substr(Auth::user()->name, 0, 1) }}
                                    </div>
                                @else
                                    <div
                                        class="avatar bg-primary text-white rounded-circle d-flex align-items-center justify-content-center fw-bold shadow-sm"
                                        style="width: 32px; height: 32px; font-size: 14px;">
                                        {{ substr(Auth::user()->name, 0, 1) }}
                                    </div>
                                @endif
    ...
    <main class="py-5 mt-5 flex-grow-1">
        <div class="container mt-3">
            @if(session('success'))
                <div
                    class="alert alert-success alert-dismissible fade show bg-success bg-opacity-10 text-success border-success border-opacity-25 rounded-pill px-4 animate-fade-in-up"
                    role="alert">
            ...
            @if(session('error'))
                <div
                    class="alert alert-danger alert-dismissible fade show bg-danger bg-opacity-10 text-danger border-danger border-opacity-25 rounded-pill px-4 animate-fade-in-up"
                    role="alert">
            ...
            @if($errors->any())
                <div
                    class="alert alert-danger alert-dismissible fade show bg-danger bg-opacity-10 text-danger border-danger border-opacity-25 rounded-3 mb-3"
                    role="alert">
                    <ul class="mb-0">
            ...
            <div class="text-center text-md-end">
                ...
                @if($contactEmail)
                    <p class="mb-0 small">По всем вопросам: <a href="mailto:{{ $contactEmail }}"
                                                               class="text-white-50 hover-text-white">{{ $contactEmail }}</a>
                    </p>
                @endif
            </div>
        </div>
